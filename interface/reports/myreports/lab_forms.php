...
// Modified - 2013-07-01 Ron Criswell <Williams Medical Technology>
// Added support of the Quest interface orders and results.
...
$query = "SELECT " .
  ...
  "WHERE " .
  "forms.deleted != '1' AND patient_data.fname != '#ORPHAN#' AND " .
// 2013-02-14 R.CRISWELL
//  "forms.formdir = 'whc_lab' ";
  " (forms.formdir = 'whc_lab' OR forms.formdir LIKE 'quest%' OR forms.formdir LIKE 'lab%') ";
// ---------------------
if ($form_from_date) {
      ...
      <table class='text'>
         <tr>
          ...
          <td><?php
               // Build a drop-down list of form statuses.
              $query = "SELECT option_id, title FROM list_options WHERE ".
                "list_id = 'Lab_Special_Handling' ORDER BY seq";
              $ures = sqlStatement($query);
...
if ($res) {
  ...
  while ($row = sqlFetchArray($res)) {
    ...
    if ($row['formdir'] == 'quest_result' || $row['formdir'] == 'quest_order'
    		|| $row['formdir'] == 'labcorp_result' || $row['formdir'] == 'labcorp_order') {
    	...
    	if ($form_special > 0) {
    		...
    		if ($row['formdir'] == 'labcorp_order' && $farray['request_handling'] != $form_special) continue;
    		if ($row['formdir'] == 'labcorp_result' && $farray['result_handling'] != $form_special) continue;
    	}
    }
	else {    
	    if($form_status && ($form_status != $fstatus)) continue;
    ...
    if($form_lab && ($form_lab != $farray['wl_lab'])) {
    	if (($row['formdir'] == 'quest_result' || $row['formdir'] == 'quest_order') && $form_lab != 'q')
	    	continue;
    	if (($row['formdir'] == 'labcorp_result' || $row['formdir'] == 'labcorp_order') && $form_lab != 'l')
	    	continue;
    }
    // ---------------------
 ...
 <?php 
    // 2013-02-14 R.CRISWELL
   	...
   	if ($row['formdir'] == 'labcorp_order')
   		$link_ref="$rootdir/forms/labcorp_order/update.php?id=$this_id&pid=".$row['ppid']."&enc=".$row['encounter'];
   	if ($row['formdir'] == 'labcorp_result')
   		$link_ref="$rootdir/forms/labcorp_result/update.php?id=$this_id&pid=".$row['ppid']."&enc=".$row['encounter'];
   	// ---------------------
  ?>
