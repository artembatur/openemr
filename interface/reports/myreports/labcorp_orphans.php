...
//
$ORDERHASH = array(
  'doctor'  => 'lower(u.lname), lower(u.fname), lr.specimen_datetime',
  'patient' => 'lower(lr.request_pat_last), lower(lr.request_pat_first), lr.specimen_datetime',
  'order'  => 'lower(lr.request_order), lr.specimen_datetime',
  'time'    => 'lr.specimen_datetime, lower(u.lname), lower(u.fname)',
  'status'    => 'lr.status, lr.specimen_datetime, lower(u.lname), lower(u.fname)',
);

$SITEHASH = array();
$sres = sqlStatement("SELECT * FROM list_options WHERE list_id = 'LabCorp_Site_Identifiers' ");
...
// hide a result
if ($form_ignore) {
	sqlStatement("UPDATE form_labcorp_result SET status='h' WHERE id = '".$form_ignore."' ");
	$form_ignore = '';
}
...
$query = "SELECT lr.*, u.lname AS ulname, u.fname AS ufname, u.mname AS umname, u.username AS username FROM form_labcorp_result lr " .
	"LEFT JOIN users u ON lr.request_provider = u.id WHERE lr.request_id = 0 ";
if ($form_from_date) {
...
if ($form_status) {
	$query .= "AND (lr.status = 'u' OR lr.status = 'h') ";
}
else {
	$query .= "AND lr.status = 'u' ";
}
...
<script LANGUAGE="JavaScript">
 ...
 function dosearch(id) {
	 url = "<?php echo $webroot ?>/interface/forms/labcorp_result/link_result.php?id=" + id;
...
<span class='title'><?php xl('Report','e'); ?> - <?php xl('LabCorp Orphan Results','e'); ?></span>

<div id="report_parameters_daterange">
<?php echo date("d F Y", strtotime($form_from_date)) ." &nbsp; to &nbsp; ". date("d F Y", strtotime($form_to_date)); ?>
</div>

<form method='post' name='theform' id='theform' action='labcorp_orphans.php'>
...
if ($res) {
  ...
  while ($row = sqlFetchArray($res)) {
    $docname = '';
    if ($row['username'] == 'labcorp' || $row['username'] == '') {
    	$docname = '[ NO PROVIDER ]';
    }
    else {
