...
$ORDERHASH = array(
	...
	'patient' => 'lower(lab.pat_last), lower(lab.pat_first), lab.result_datetime',
	'pubpid'  => 'CAST(lab.pid as SIGNED INTEGER) ASC, lab.result_datetime',
	'time'    => 'lab.result_datetime, lower(u.lname), lower(u.fname)',
);
...
if ($_POST['form_refresh'] || $_POST['form_orderby']) {
	...
	$query .= "lab.pid, lab.pubpid, lab.status, lab.pat_last, lab.pat_first, lab.pat_middle, lab.order_number, lab.order_datetime, lab.result_datetime ";
	$query .= "FROM forms f, form_encounter fe, ";
	$subs = array();
	if ($quest && (!$form_lab || $form_lab == 'q')) {
		$sub = "SELECT id, request_pid AS pid, request_pubpid AS pubpid, status, request_pat_last AS pat_last, request_pat_first AS pat_first, request_pat_middle AS pat_middle, request_order AS order_number, result_datetime, specimen_datetime AS order_datetime, request_provider FROM form_quest_result ";
		$sub .= "WHERE activity = 1 ";
		if ($form_from_date) $sub .= "AND result_datetime >= '$form_from_date 00:00:00' AND result_datetime <= '$form_to_date 23:59:59' ";
		if ($form_provider) $sub .= "AND request_provider = '$form_provider' ";
		if ($form_special) $sub .= "AND result_handling = '$form_special' ";
		$subs[] = $sub;
	}
	if ($labcorp && (!$form_lab || $form_lab == 'l')) { 
		$sub = "SELECT id, request_pid AS pid, request_pubpid AS pubpid, status, request_pat_last AS pat_last, request_pat_first AS pat_first, request_pat_middle AS pat_middle, request_order AS order_number, result_datetime, specimen_datetime AS order_datetime, request_provider FROM form_labcorp_result ";
		$sub .= "WHERE activity = 1 ";
		if ($form_from_date) $sub .= "AND result_datetime >= '$form_from_date 00:00:00' AND result_datetime <= '$form_to_date 23:59:59' ";
		if ($form_provider) $sub .= "AND request_provider = '$form_provider' ";
		if ($form_special) $sub .= "AND result_handling = '$form_special' ";
	...
	$query .= "WHERE f.deleted != '1' AND f.formdir LIKE '%_result' AND f.encounter = fe.encounter ";
...
<html>
	...
	<body class="body_top">
		...
		<span class='title'><?php xl('Report','e'); ?> - <?php xl('Lab Results','e'); ?></span>

		<div id="report_parameters_daterange">
			<?php echo date("d F Y", strtotime($form_from_date)) ." &nbsp; to &nbsp; ". date("d F Y", strtotime($form_to_date)); ?>
		</div>

		<form method='post' name='theform' id='theform' action='labcorp_results.php'>
...
<?php
	...
	$query = "SELECT option_id, title FROM list_options WHERE list_id = 'Lab_Form_Status' AND title LIKE 'Result_%' ORDER BY seq";
	$ures = sqlStatement($query);
...
<?php
	// Build a drop-down list of form statuses.
 	$query = "SELECT option_id, title FROM list_options WHERE list_id = 'Lab_Special_Handling' ORDER BY seq";
	$ures = sqlStatement($query);
