...
class wmtInsurance {
	...
	public function __construct($id = false) {
		...
		$query = "SELECT a.*, i.*, i.id AS company_id, c.name AS company_name, c.id AS company_id, c.freeb_type AS plan_type FROM insurance_data i ";
	...
	public static function getCompany($provider) {
		...
		if ($provider == 'self') {
			...
			$query .= "WHERE ic.id = '$provider' LIMIT 1 ";
			$record = sqlQuery($query);
		}
...
class wmtForm {
	...
	public static function fetchRecent($form_name, $pid, $active=TRUE) {
		...
		$query = "SELECT form_id FROM forms ";
		$query .= "WHERE formdir = '$form_name' AND pid = '$pid' ";
		if ($active) $query .= "AND deleted = 0 ";
		$query .= "ORDER BY date DESC LIMIT 1";

		$data = sqlQuery($query);
		$object = new wmtForm($form_name,$data['form_id']);
